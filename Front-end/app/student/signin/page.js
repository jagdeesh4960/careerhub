...
import { asynctstudentsignin } from "@/store/Actions/studentAction";
...
const Page = () => {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!studentLoginForm.email.trim()) {
      newErrors.email = "Email is required";
    } else if (!/^\S+@\S+\.\S+$/.test(studentLoginForm.email)) {
      newErrors.email = "Invalid email format";
    }
    if (!studentLoginForm.password) {
  ...
  const SigninHandler = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        await dispatch(asynctstudentsignin(studentLoginForm));
      } finally {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-50 to-zinc-100 px-4">
